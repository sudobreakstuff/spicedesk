...
class _ReportsScreenState extends ConsumerState<ReportsScreen> {
  ...
  Future<void> _printInvoice(Map<String, dynamic> invoice, Map<String, dynamic> sale, Map<String, dynamic>? cust, DateTime dt, double total, List<Map<String, dynamic>> items) async {
    ...
    doc.addPage(pw.MultiPage(
      ...
      build: (context) => [
        pw.Header(level: 0, child: pw.Text('INVOICE', style: const pw.TextStyle(fontSize: 28, fontWeight: pw.FontWeight.bold))),
        pw.SizedBox(height: 8),
        pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
          pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.start, children: [
            pw.Text('SpiceDesk', style: const pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
            pw.Text('Invoice #$invNum', style: const pw.TextStyle(fontSize: 12)),
          ]),
          pw.Column(crossAxisAlignment: pw.CrossAxisAlignment.end, children: [
            pw.Text('Date: ${DateFormat('dd MMM yyyy').format(dt)}', style: const pw.TextStyle(fontSize: 12)),
            pw.Text('Customer: ${cust?['name'] ?? 'Walk-in'}', style: const pw.TextStyle(fontSize: 12)),
            pw.Text('Payment: ${sale['payment_method'] ?? ''}', style: const pw.TextStyle(fontSize: 12)),
          ]),
        ]),
        ...
        ...items.expand((i) => [
          pw.Row(children: [
            pw.Expanded(flex: 3, child: pw.Text(i['product_name'] ?? '', style: const pw.TextStyle(fontSize: 11))),
            pw.Expanded(flex: 1, child: pw.Text('${(i['quantity'] as num?)?.toInt() ?? 0}', style: const pw.TextStyle(fontSize: 11))),
            pw.Expanded(flex: 2, child: pw.Text('R ${((i['unit_price'] as num?)?.toDouble() ?? 0).toStringAsFixed(2)}', style: const pw.TextStyle(fontSize: 11))),
            pw.Expanded(flex: 2, child: pw.Text('R ${((i['line_total'] as num?)?.toDouble() ?? 0).toStringAsFixed(2)}', style: const pw.TextStyle(fontSize: 11))),
          ]),
        ]),
        pw.SizedBox(height: 24),
        pw.Container(
          alignment: pw.Alignment.centerRight,
          child: pw.Text('TOTAL: R ${total.toStringAsFixed(2)}', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
        ),
